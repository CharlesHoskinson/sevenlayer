...
<!-- Revision ledger -->
<!--
=== v2 (P1 fixes, inherited) ===
P1-1 (ch06-fiat-shamir-vulnerabilities, B): Added citations for all three incidents.
P1-2 (ch06-nightstream Lean boundary, D): Near-verbatim repetition merged.
P1-3 (ch06-recursion-vs-folding, A): Zexe citation corrected.
P1-4 (ch06-the-folding-genealogy, A): LatticeFold venue confirmed as ASIACRYPT 2025.
P1-5 (ch06-the-folding-genealogy, B): ProtoGalaxy authors confirmed as Eagen and Gabizon, ePrint 2023/1106.
P1-6 (ch06-the-hybrid-pipeline, B): Concrete timing/cost figures sourced.

=== v3 (P2 fixes) ===
P2-A (ch06-case-study-midnight-s-sealed-certificate, A): Table cell for Neo/Symphony "Proof time" changed from "GPU-accelerated NTT" to "Sub-second (GPU, NTT-based; Symphony target)" to give a comparable duration indication rather than a mechanism description.

P2-B-midnight (ch06-case-study-midnight-s-sealed-certificate, B): Performance table note updated: "measured on development hardware; production performance may differ; no public benchmark link is available as of this writing" — makes the sourcing limitation explicit.

P2-C-circle (ch06-circle-starks-and-stwo-a-generational-leap, C): "The key insight is a change in the algebraic structure" — removed. Section rewritten to open "The change is in the algebraic structure..." (AI smell excised; no meaning lost).

P2-B-icicle (ch06-circle-starks-and-stwo-a-generational-leap, B): ICICLE speedup range "3.25x to 7x" now cited: "[ICICLE-Stwo benchmarks, ingonyama.com/icicle/stwo]".

P2-B-940x (ch06-circle-starks-and-stwo-a-generational-leap, B): 940x claim now tagged with both the Circle STARKs ePrint (2024/278) and the StarkWare blog post as primary sources. Added sentence noting the claim is single-source and awaits independent replication.

P2-A-gnark (ch06-fiat-shamir-vulnerabilities, A): "used by multiple Ethereum rollups" softened to "used by at least one Ethereum rollup in production; the advisory does not name specific deployments affected."

P2-E-speedrace (ch06-from-speed-race-to-security-race, E): Section extended by ~120 words with a developed treatment of the random oracle model gap — the substantive new content mentioned in the finding. This provides material beyond what appears in ch06-real-time-ethereum-proving.

P2-B-speedrace (ch06-from-speed-race-to-security-race, B): EF announcement now tagged: "[Ethereum Foundation, 'Ethereum proving roadmap update,' December 2025]".

P2-A-twist (ch06-nightstream-what-a-folding-engine-looks-like-from-the-inside, A): "Twist and Shout memory arguments" clarified: "The memory arguments (named 'Twist' and 'Shout' in the Nightstream codebase, following the protocol names used there)..." — distinguishes codebase name from editorial invention.

P2-B-nightstream (ch06-nightstream-what-a-folding-engine-looks-like-from-the-inside, B): Added link to Nightstream's public GitHub repository (https://github.com/LFDT-Nightstream/Starstream) in opening paragraph so readers can verify crate names and architecture.

P2-B-realtime (ch06-real-time-ethereum-proving, B): "Two additional teams" replaced by "at least two teams crossed the threshold with named, public results" — the vague second claim removed; only SP1 Hypercube and ZKsync Airbender are named.

P2-A-sp1 (ch06-real-time-ethereum-proving, A): SP1 Hypercube description corrected: "built entirely on the sumcheck protocol" → "built on the sumcheck protocol" with parenthetical note that the system also uses a STARK wrapper and Groth16 for on-chain posting.

P2-C-fatal (ch06-recursion-vs-folding-russian-dolls-and-snowballs, C): "This is the kind of obstacle that looks fatal until someone sees through it" — removed. Paragraph restructured to open directly with Nova's move.

P2-C-worthunderstanding (ch06-recursion-vs-folding-russian-dolls-and-snowballs, C): "It is worth understanding why" — not present in source (does not appear in the Russian Dolls section verbatim); checked: no change needed. [ALREADY ABSENT in source — skipped.]

P2-C-worthspellingout (ch06-sealing-the-certificate, C): "it is worth spelling out" — not present in the Sealing the Certificate intro section; appears only in The Hybrid Pipeline and The Three Families sections (covered by P2-C-hybrid and P2-C-threefamilies below). Intro section is clean as noted in the brief. [ALREADY CLEAN — skipped.]

P2-B-schwartz (ch06-sealing-the-certificate, B): Schwartz-Zippel cited in the running example box with forward reference to Ch10 noted explicitly: "(Schwartz-Zippel is a standard tool in algebraic complexity; see Chapter 10 for the formal statement alongside the discussion of KZG, FRI, and Ajtai commitments.)"

P2-D-crossover (ch06-snark-recursion-vs-folding-the-full-picture, D): The 1,000–10,000 crossover threshold appeared twice (once mid-section as a self-contained sentence, once in the closing paragraph). Removed standalone mid-paragraph restatement; single occurrence remains in the closing paragraph with full context.

P2-B-mangrove (ch06-snark-recursion-vs-folding-the-full-picture, B): Mangrove citation updated: "Nguyen, Datta, Chen, Tyagi, and Boneh, 2024; author order as listed in the ePrint; a proceedings citation should be verified against the published version" — flags need for ePrint number and author-order verification.

P2-A-symphony (ch06-the-folding-genealogy, A): Symphony (2026) parenthetical added: "(Symphony is a 2026 result; as of this writing it circulates as a preprint and has not yet appeared in formal proceedings.)"

P2-B-sumcheck (ch06-the-folding-genealogy, B): Sumcheck attribution clarified: "introduced by Lund, Fortnow, Karloff, and Nisan (FOCS 1990; JACM 1992)" — distinguishes conference and journal versions.

P2-A-polygon (ch06-the-hybrid-pipeline, A): "All major production systems follow this architecture: SP1, Stwo, Polygon, ZKM" replaced by qualified statement: "This architecture is followed by SP1 (Succinct Labs), Stwo (StarkWare), and ZKM, among others. (Polygon runs multiple proving systems -- Polygon zkEVM uses a different pipeline -- so 'all major production systems' would overstate the uniformity; the pattern is dominant, not universal.)"

P2-C-hybrid (ch06-the-hybrid-pipeline, C): "The implication is worth spelling out" — not found verbatim in The Hybrid Pipeline section; the phrase "worth spelling out" does not appear in this section in the source. Checked v2 text: not present. [ALREADY ABSENT — skipped.]

P2-A-gasprice (ch06-the-hybrid-pipeline, A): "$0.50–$1.00 at typical gas prices" → "$0.50 to $1.00 at mid-2025 gas prices (Ethproofs tracks live per-proof cost; the figure fluctuates with gas market conditions)."

P2-A-quantum (ch06-the-post-quantum-horizon, A): "Becomes insecure. Not 'might become insecure.' Becomes insecure." — hedged. Rewritten as "Once a sufficiently powerful quantum computer exists, Shor's algorithm breaks the discrete logarithm problem... A fault-tolerant, cryptographically relevant quantum computer does not yet exist, and estimates for when one will vary widely." Separates mathematical implication from practical threat.

P2-C-bolding (ch06-the-three-families, C): Bold labels on the Three Envelopes subsection paragraphs ("**Groth16 is the smallest possible envelope.**" etc.) removed. Prose restructured to open with plain text, removing the AI-smell numbered-proof-steps feel while retaining all content.

P2-C-worthspelling-threefamilies (ch06-the-three-families, C): "It is worth spelling out" — not found verbatim in The Three Families section. Checked source: phrase does not appear in that section. [ALREADY ABSENT — skipped.]

P2-A-bn254 (ch06-the-three-families, A): BN254 security claim "~100 bits after Tower NFS advances" now cites Kim and Barbulescu, CRYPTO 2016, inline. Same citation added in the Midnight case study section.
-->
