...
Revisions in this draft, against the 9 P1 findings list:

- P1 #1 (Sudoku arithmetic). 4 rows + 4 columns + 4 (2x2 boxes) = 12 groups, each with C(4,2) = 6 pairs, giving 12 x 6 = 72 uniqueness pair checks. Total constraints now stated as 16 range + 8 given-cell + 72 uniqueness = 96, explicitly walked through for the reader. The R1CS expansion estimate was revised proportionally: 16 degree-4 range checks decompose into ~3 bilinear rows each (~48), plus 72 uniqueness pair-difference rows, plus 8 equality rows, landing at roughly 130 R1CS constraints (the earlier "~120" figure was consistent with the old "72 constraints" baseline; with 96 baseline constraints the expanded figure rises accordingly). The "~120" phrase is replaced with "roughly 130".

- P1 #2 (Groth16 proof size). Both the body and the summary now state 192 bytes on BLS12-381 ("two $\mathbb{G}_1$ points and one $\mathbb{G}_2$ point, compressed: 2*48 + 96 = 192 bytes"). The inline "~128 bytes" and the Reference Data "128-byte proofs" phrasing are gone. Source added to the Sources list deriving the 192-byte figure from the BLS12-381 curve specification.

- P1 #3 (AIR table misleading). The "Same Computation, Three Encodings" table was restructured. The old column "Constraints for 1000 hash rounds" is split into two columns -- "Description size (1000 hash rounds)" and "Prover trace work (1000 hash rounds)" -- with an explicit note above the table that description size measures what the circuit designer writes, not what the prover computes. For AIR, description stays at ~30 polynomials while trace grows to ~30,000 rows. The clarifying paragraph after the table was tightened to match: AIR compresses the description, not the prover's work.

- P1 #4 (Jolt 64-bit canonical). The Jolt section now fixes the 64-bit RISC-V case as canonical: 6 chunks x 3 field elements = 18 field elements per instruction. The 32-bit case (4 chunks, 12 field elements) is mentioned once as a contrast, then not reused. The 32-bit ADD walkthrough was rewritten as a 64-bit ADD walkthrough so the 18-element figure matches the worked example. Reference Data updated accordingly ("~18 field elements per 64-bit RISC-V instruction").

- P1 #5 (LogUp-GKR attribution). First mention of Plookup now reads "Ariel Gabizon and Zachary Williamson introduced Plookup in 2020 (ePrint 2020/315)." First mention of LogUp-GKR now reads "Shahar Papini and Ulrich Haboeck combined LogUp with the GKR interactive proof protocol (ePrint 2023/1284)". Sources list updated with full author names for both, consistent with the Sources section conventions.

- P1 #6 (impact opcode). The I/O bullet now reads: "`private_input` and `public_input` pull values into the circuit; `output` exposes a value as a public output. `impact` represents a ledger-state operation -- reading from or writing to Midnight's on-chain state -- binding circuit values to the ledger so that a proof can assert and update persistent on-chain data as part of its execution." All four I/O opcodes are now defined in one place.

- P1 #7 (BLS12-381 field width). Body, range-check discussion, Reference Data, and the standalone "BLS12-381 Field Consequence" subsection all now say "a 255-bit prime with order approximately $2^{255}$." The "~$2^{253}$" and "~255 bits" mismatch is gone. The example talking about substituting "any 253-bit field element" is now "any 255-bit field element," matching the corrected width and the Ch2 draft.

- P1 #8 (Airbender row). The table's overhead column now states "vs ~100 ms block" explicitly in the header. The Airbender row is recomputed as 35 s / 100 ms = ~350x, matching the block-baseline treatment of the other rows. Prose before the table now explains that the earlier "~8,000x" figure used a different (single-transaction) native baseline and is not comparable to the block-level numbers in this table. The other rows in the table were also rebaselined to ~100 ms for internal consistency (previously the column mixed implicit baselines).

- P1 #9 (Midnight shielded transfer). Body and table both now use the 20 s / 20 us = 1,000,000x figure and explicitly name the 20 microsecond native baseline as "the arithmetic of a handful of balance operations running on a modern CPU." The earlier table entry of "~4,000,000x" is replaced with "~1,000,000x (arithmetic baseline)." The alternative 1,000x number, measured against the full security computation rather than raw arithmetic, is retained in the surrounding prose as a second way of viewing the same transaction.

Unresolved / caveats:
- The rebaselining of the overhead table in P1 #8 also affects the other rows (RISC Zero, SP1 v1, SP1 Hypercube, Stwo). The original table's implicit baselines were inconsistent: the 8,000x/10,000x/etc. figures did not all use 100 ms as the native baseline. The ledger notes this; the table now uses 100 ms block throughout and the pre-existing multipliers were adjusted down accordingly (600x, 150x, 70x, 100x, 350x). If a prior chapter quoted these multipliers verbatim, those references may need a follow-up pass.
- Sources added: ZKsync Airbender announcement (bibliography entry 53 already present). No new bibliography entries required beyond the derivation-of-192-bytes note in the Sources section.
