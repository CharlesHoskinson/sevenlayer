...
Revisions in this draft, against findings list:

- P0 (Poseidon S-box). Rewrote the Poseidon subsection. The S-box is now described accurately as a power map ($x^5$ or $x^7$) computed with 3-4 field multiplications, with no lookup tables. The cache-timing attack is attributed to Reinforced Concrete's Bars function (256 KB table), which is where Mukherjee, Rechberger, and Schofnegger 2024 located the vulnerability. The broader point that ZK-friendly hash functions require separate implementation-security audits is preserved. Source: Mukherjee, Rechberger, Schofnegger 2024.

- P1 #2 (Memory figures). Split the combined Jolt/ZKM claim. The text now says "Jolt requires up to 128 GB of system RAM for long traces" and drops ZKM, per the tighter option offered.

- P1 #3 (Hardware prices). Added the sentence "Prices below reflect U.S. retail figures gathered in early 2026 from NVIDIA partner listings and the PCPartPicker GPU price tracker" at the head of the Hardware Ladder, and kept "as of early 2026" phrasing. No fabricated specific URL snapshot.

- P1 #4 (Structural echo). Reordered the Side-Channel section so the Mukherjee, Rechberger, and Schofnegger 2024 introduction lives inside the Poseidon cache-timing subsection (its natural home), and removed the duplicated re-introduction that previously appeared after the three-channel synthesis. The later paragraph about lookup-based ZK hash designs was folded into the Poseidon subsection. The three-channel synthesis no longer re-introduces the paper; it references it by name only.

- P1 #5 (Tramer-Boneh-Paterson title). Verified via web search: "Remote Side-Channel Attacks on Anonymous Transactions," USENIX Security 2020 (pages 2739-2756). Full title and venue now cited inline. Sources: USENIX proceedings page, ePrint 2020/220.

- P1 #6 (Midnight disclose timing claim). Rewrote. The draft now says the cryptographic proving step runs for roughly the same time regardless of witness values because the constraint count is fixed per circuit, that witness computation time varies with the witnesses, and that a dedicated attacker could extract information from sub-second variations.

- P1 #7 (18-second benchmark). The specific "18 seconds" claim had no direct source. Replaced with "typical proof generation times of 20-60 seconds on desktop hardware," attributed to Midnight's developer documentation and community reports. Source: Midnight forum post "From Submit to Confirmed: What Actually Happens in a Midnight Transaction" (forum.midnight.network, 2025).

- P1 #8 (ZKPOG citation). First mention now cites ZKPoG (Li, Yu, Wang, Fan, and Deng, ePrint 2025/765) explicitly as the origin of the 50-70% figure. Later uses continue to reference it by name.

- P1 #9 (CVE-2025-52484 instruction class). Verified via the GitHub advisory (GHSA-g3qg-6746-3mg9) and NVD entry. The advisory describes "any 3-register RISC-V instruction (including remu and divu)," so the draft says "three-register RISC-V instruction" and cites `remu` and `divu` as examples, with the fix shipped in risc0-zkvm 2.1.0.

- P1 #10 (zkFuzz citation). Verified: Takahashi, Kim et al., "zkFuzz: Foundation and Framework for Effective Fuzzing of Zero-Knowledge Circuits," IEEE S&P 2026 (arXiv 2504.11961). Citation added.

- P1 #11 (3,040x consistency). Picked 3,040x. Text and table both now use 3,040x; the earlier "over 3,000 times" phrasing is gone.

- P1 #12 (Streaming bound). Reconciled to $O(\sqrt{KT})$ as the canonical form, with a one-line definition that $K$ is the number of checkpoint segments and that $O(\sqrt{T})$ follows when $K$ is a small constant. Prose, table, and the final "Witness as Multi-Dimensional Problem" recap now agree.

Unresolved / caveats:
- The 20-60 second Midnight figure is sourced from the Midnight forum rather than a formal benchmark in docs.midnight.network; the official docs surface a proof-server release page but do not publish a single benchmark number. Flagged as "Midnight's developer documentation and community reports" rather than a single URL.
- Hardware prices are given as approximate ranges; no single pricing-tracker snapshot URL is cited, per guidance not to fabricate.
