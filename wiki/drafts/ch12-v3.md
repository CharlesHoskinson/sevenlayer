...
P0 ch12-five-lessons-for-zk-system-design (A): Lesson 3 previously stated "fees are paid in a shielded token generated by time-locked staking." DUST is explicitly unshielded (public) per ch12-midnight-at-a-glance and ch08. Revised Lesson 3 to: "fees are paid in DUST -- a public (unshielded) fee token generated from time-locked NIGHT staking." Rewrote the supporting paragraph so the privacy argument rests on the generation mechanism (time-locked staking), not on a false "shielded" label.

P1 ch12-full-seven-layer-mapping (A) -- BLS12-381 bitlength. Layer 6 previously read "All ZKIR values are elements of the BLS12-381 scalar field ($\sim 2^{253}$)." Replaced with "a 255-bit prime scalar field $r \approx 2^{254}$, with a 381-bit base field $p$."

P1 ch12-full-seven-layer-mapping (A) -- Halo 2 vs UltraPlonk. Layer 5 revised to "Halo 2 (a PLONK-family proof system with UltraPlonk-style custom-gate arithmetization)."

P1 ch12-full-seven-layer-mapping (D) -- Verifier key immutability. Layer 7 and Privacy Theater Analogy rewritten to reflect contract-level (not contract-internal) mutability model.

P1 ch12-the-privacy-theater-analogy (A) -- "air-gapped" corrected to "wall of topology, not of isolation."

Three-token table label updated from "(Night/Shielded/DUST)" to "(NIGHT/DUST/custom)."

v3 fixes (P2):

P2-1 ch12-five-lessons-for-zk-system-design (B): L2Beat reference now includes the framework URL (l2beat.com/scaling/projects) and clarifies the assessment is applied "by analogy." Cross-contract failure and compiler bug now cite documentation version: MidnightJS SDK Reference v0.14 §7.3 and Compact Language Reference v0.14 §4.8.

P2-2 ch12-five-lessons-for-zk-system-design (B): Same fix as P2-1 above -- defect claims anchored to specific doc versions to flag staleness risk.

P2-3 ch12-full-seven-layer-mapping (A): Layer 7 "~490 trillion SPECK per circuit call" now reads "approximately 490 trillion SPECK per circuit call (SPECK being the smallest sub-unit of DUST, analogous to satoshis for Bitcoin; per the Wallet SDK Reference §4.2)." SPECK defined in-line.

P2-4 ch12-full-seven-layer-mapping (B): Pluto-Eris curve history now cites Developer Guide p.12. Halo 2 backend rationale now cites Developer Guide pp.14-16.

P2-5 ch12-midnight-at-a-glance (B): Added sentence sourcing `localhost:6300` to MidnightJS SDK Reference §3.1 and per-UTXO shielded/unshielded distinction to Wallet SDK Reference §2.4.

P2-6 ch12-the-privacy-theater-analogy (B): "~490 trillion SPECK per circuit call" reworded to "approximately 490 trillion SPECK per circuit call, where SPECK is DUST's smallest denomination." SPECK defined in-line; sourcing is via the Layer 7 paragraph where the full citation appears.

P2-7 ch12-where-midnight-challenges-the-model (A): ALREADY RESOLVED IN V2. Table previously read "(Night/Shielded/DUST)"; v2 corrected to "(NIGHT/DUST/custom)." No change needed.

P2-8 ch12-where-midnight-challenges-the-model (B): "nanopass architecture spanning 26 intermediate languages" now cites Compact Language Reference §6.1.

P2-9 ch12-where-midnight-validates-the-model (E): Added paragraph after the five validation points explaining why Layers 3 and 6 resist clean mapping: L3 is absorbed into the compiler's disclosure analysis (which straddles L2 and L4); L6 is intrinsic to L1's curve choice (Jubjub and Poseidon exist because BLS12-381 was chosen).
