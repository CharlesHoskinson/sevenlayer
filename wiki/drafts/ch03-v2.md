...
1. **Cairo/Goldilocks field (Evolution section).** Replaced the claim that Cairo "adopted the Goldilocks prime field" with the correct Stark prime ($p = 2^{251} + 17 \cdot 2^{192} + 1$). Added the large-field -> small-field historical arc (Goldilocks in Plonky2/Plonky3/RISC Zero, BabyBear in SP1, M31 in Stwo).
2. **Chaliasos SoK citation (Evolution section).** Added full citation at first use in this chapter: Chaliasos et al., "SoK: What Don't We Know? Understanding Security Vulnerabilities in SNARKs," USENIX Security 2024 (bibliography #3). Reused at the 67% figure in the intro and at the 95/141 figure in the under-constrained section.
3. **ZKIR 24/8 enumeration (Midnight section).** Replaced the incomplete 17-opcode list with the full 24-opcode enumeration across 8 categories: arithmetic (3) + constraints (4) + comparison (2) + control flow (2) + type encoding (3) + division (1) + cryptographic (5) + I/O (4) = 24.
4. **RISC-V adoption + 67% citation (intro).** Softened "eight of ten major zkVMs" to "most of the zero-knowledge virtual machines in production" and added an explicit landscape paragraph pointing to Chapter 11 (SP1, RISC Zero, Jolt, Airbender, ZisK, Pico Prism for RISC-V; Stwo on Cairo). Kept the 67% figure and cited Chaliasos et al. 2024 directly at first use.
5. **Duplicate opening (Developer section).** Deleted the duplicated "taxonomy of philosophies describes how architects think..." paragraph; kept the version that continues into the lifecycle.
6. **LLVM 40%/1-4% (Developer section).** Rewrote the paragraph to distinguish per-benchmark max (45%, >40%) from suite-wide average (1-4%) and cited Gassmann et al., arXiv 2508.17518, 2026 (bibliography #32).
7. **ZK-Coder + CirC citations (Developer section).** Cited ZK-Coder as Xue et al., "From Evaluation to Enhancement: LLMs for ZK Proof Code Generation," arXiv 2509.11708, 2026 (confirmed 20% -> 88% figures match paper). Cited CirC as Ozdemir, Brown, and Wahby, IEEE S&P 2022, ePrint 2020/1586 (bibliography #34).
8. **Cairo/Goldilocks (Philosophy B).** Corrected to Stark prime ($p = 2^{251} + 17 \cdot 2^{192} + 1$); clarified that Goldilocks / BabyBear / M31 are small-field *successors* to Cairo's large-field STARK.
9. **Philosophy D privacy column (summary table).** Split the "Compiler-enforced" cell into three distinct models: Compact = compiler-enforced (disclosure analysis); Noir = annotation-based (developer responsibility); Leo = structural (record model).
10. **Airbender + R0VM 2.0 + Aztec Ignition citations.** Airbender 21.8M cycles/s on H100 cited to ZKsync product announcement, June 2025 (bibliography #53). R0VM 2.0 softened to "order-of-magnitude improvements... from roughly 35 minutes to under a minute" with citation to RISC Zero's "Introducing R0VM 2.0" blog, April 2025 (https://risczero.com/blog/introducing-R0VM-2.0). Aztec Ignition softened from "first decentralized L2" to "a decentralized L2... with 185+ operators across five continents and thousands of sequencers," cited to Aztec's "Ignition Chain Update" (https://aztec.network/blog/aztec-ignition-chain-update).
11. **Philosophy D synthesis paragraph order.** Reordered so the A/B/C unification paragraph comes first ("shared assumption: developer doesn't think about privacy"), then the Philosophy D synthesis (the three languages' differing routes), then the closing contrast line.
12. **Plonky3 / Stwo correction.** Removed "Plonky3 (used by SP1 and Stwo)"; replaced with "Plonky3 (used by SP1)" and added a sentence noting Stwo uses Circle STARKs over M31, not Plonky3.
13. **Picus / ZKAP / zkFuzz / MTZK citations.** Added full citations: Picus/QED^2 -> Pailoor et al., PLDI 2023, ePrint 2023/512 (bibliography #12). ZKAP -> Wen et al., USENIX Security 2024, ePrint 2023/190 (bibliography #13). zkFuzz -> Takahashi et al., IEEE S&P 2026, arXiv 2504.11961 (confirmed 66 bugs / 38 zero-days). MTZK -> Xiao et al., NDSS 2025 (confirmed 21 bugs across 4 industrial compilers, 15 patched).
