...
P3 findings resolved in this draft (v4), against ch04.md brief:

1. **[ch04-execution-traces](E)** Out-of-order/speculation note: added closing paragraph to Execution Traces explaining why neither out-of-order execution nor ORAM-based approaches break the sequential dependency. The sequentiality is inherent to what witness generation is, not an artifact of naive implementation.

2. **[ch04-execution-traces](D)** Forward pointer on trace-generation cost: changed "And generating the recording -- running the computation while capturing every detail -- is the expensive part." to a two-sentence forward: "The scale of that difference is the subject of the next section; generating the recording is the expensive part."

3. **[ch04-memory-the-binding-constraint](D)** NTT bandwidth paragraph moved: deleted the standalone NTT paragraph at the end of the Memory section ("The memory constraint also interacts with NTT performance..."); integrated its substance into the Tier 3 description where it belongs, since the H100's HBM3 bandwidth is the reason Tier 3 matters for NTT-heavy workloads.

4. **[ch04-side-channel-attacks-when-the-walls-leak](E)** GPU constant-time cost gap: added "No published work has measured the performance cost of constant-time proving on GPU hardware end-to-end; quantifying this tradeoff is an open problem." in the SIMT thread-divergence paragraph, before "What is clear is that any system claiming both GPU-accelerated proving and zero-knowledge must address this tension explicitly."

5. **[ch04-the-disclose-boundary-midnight-s-witness-architecture](E)** "Segments" definition: added inline definition in the side-channel paragraph -- "the number of *segments* -- contiguous regions of the ZKIR bytecode that the ZKIR checker processes as independent units; see Chapter 5 for the ZKIR execution model --" before "in a transaction could reveal which circuit was called."

6. **[ch04-the-hidden-bottleneck](D)** Forward-reference to four gaps: added a sentence at the end of the opening section: "These problems map onto four named gaps -- Performance, Memory, Security, and Correctness -- developed in full in the final section of this chapter."

7. **[ch04-the-witness-as-a-multi-dimensional-problem](E)** Differentiate the two checklists: changed the system architect paragraph lead-in from "For the system architect, Layer 3 generates the most concrete questions in any ZK evaluation:" to "For the system architect, the four gaps translate into five design questions that sit above the hardware checklist in the Memory section -- those concern infrastructure; these concern the system as a whole:" making the scope distinction explicit.

8. **[ch04-the-witness-as-a-multi-dimensional-problem](D)** Layer numbering in closing transition: verified -- "Layer 4 must answer" in the final paragraph is correct; arithmetization is Layer 4 and the transition is from Layer 3 (this chapter) to Layer 4 (the next). No change needed; the numbering was already consistent.

9. **[ch04-witness-constraint-divergence](D)** Non-deterministic hints indexing: added forward-reference note -- "but the term *non-deterministic hint* will recur in Chapter 5's treatment of arithmetization, where the boundary between what the prover computes freely and what the constraints must enforce becomes the central design question" -- making clear this is an indexed concept with a later treatment.

10. **[ch04-witness-generation-costs](E)** BatchZK applicability scope: added "BatchZK's pipelining is not tied to a specific proof system; it operates at the infrastructure layer and can in principle wrap any prover that accepts streamed witness chunks, though current implementations target PLONK-family systems." Contrasted with the Jolt/sum-check streaming approach: "This approach is explicitly tied to the sum-check protocol used by Jolt; it does not straightforwardly transfer to SNARK systems based on polynomial commitment schemes."

P2 findings resolved in prior draft (v3): see v3 revision ledger for 24 items covering execution trace citations, Sudoku field element accounting, GSMA figures, Crypto Wars citation, TEE vulnerability descriptions, EM ZK attack framing, Zcash subsection title, Midnight timing and BIP-340, non-determinism paragraph, ZKAP F1 specificity, NAVe/Picus citations, zkSync bug description, RAM estimates with field sizes, source table years, and multiple voice/style fixes.
