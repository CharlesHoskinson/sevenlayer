...
<!-- Revision ledger
P0 #1 ch10-trust-decomposition-seven-weaker-assumptions (D): Frozen Heart count corrected from "three implementations (Bellman, Gnark, academic reference)" to "six implementations across three proof systems (Dusk PLONK, Iden3/SnarkJS Groth16, ConsenSys/gnark PLONK, ING zkrp Bulletproofs, SECBIT ckb-zkp Groth16, Adjoint Haskell Bulletproofs)" -- aligned with ch08 canonical treatment. Applied in Layer 5 thumbnail (Trust Decomposition list) and in the "If Layer 5 fails" failure-scenario paragraph.

P1 #2 ch10-path-one-hybrid-stark-to-snark-pipeline (B): Removed the incorrect "via Herodotus" attribution (Herodotus is storage-proofs, not a wrapping provider); Stwo aggregation now attributed to SHARP only, with citation [23] to Circle STARKs paper. SP1 Hypercube cited [52]. Gas-cost dollar figures softened to order-of-magnitude claim with ethproofs.org citation [44]; removed specific $5-$25 / $0.50-$1.00 figures that could not be verified at the moment of writing.

P1 #3 ch10-path-three-post-quantum-folding (A/B): FIPS citation tightened. Module-SIS now attributed specifically to FIPS 204 (ML-DSA) [64]; FIPS 203 (ML-KEM, Module-LWE) and FIPS 205 (SLH-DSA) noted for context [25]. Claim that Module-SIS underpins folding construction preserved.

P1 #4 ch10-path-two-pure-transparent (B): EF December 2025 pivot, security targets (100-bit May 2026 / 128-bit December 2026), and energy-as-metric shift now cited [55]. SP1 Hypercube proximity-gap elimination cited [52]. Brevis Pico Prism figures reframed as "Brevis reports" with the specific 16-RTX-5090 / two-machine / 99% / ~7s numbers from the Feb 2026 Brevis blog post, per P1 allowance for reported figures.

P1 #5 ch10-causal-web-dag-not-a-stack (A): "Every layer connected to every other through at most two hops" replaced with "every layer reaches every other layer through a small number of hops," which is accurate for the edge set listed (Layer 1 to Layer 3 is three hops: 1→5→4→3).

P1 #6 ch10-trust-decomposition (A): Tornado Cash Layer 2 conflation fixed. Canonical example in the "If Layer 2 fails" paragraph is now the Zcash InternalH bug (CVE-2019-7167) -- clean Layer 2 under-constrained-circuit case. Tornado Cash retained as a secondary example using the verified pre-launch MIMC `=` vs `<==` Circom constraint bug (circomlib/Kobi Gurkan disclosure), which is a genuine Layer 2 circuit bug -- not the 2023 governance attack (Layer 7), which remains cited separately in the Layer 7 thumbnail and failure scenario.

P1 #7 ch10-trustless-vs-trust-minimized (A): Closing enumeration expanded from five to all seven layers. Added Layer 4 (arithmetization faithfully encodes computation) and Layer 5 (Fiat-Shamir soundness and correct implementation) to both the italicized rhetorical question and the follow-up (a)-(g) list, matching the section's seven-assumption framing.

Length check: source ~3,810 words; revision ~4,090 words; delta +7.3%, within the ±15% budget.
-->
