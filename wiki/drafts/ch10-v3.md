...
<!-- Revision ledger
P0 #1 ch10-trust-decomposition-seven-weaker-assumptions (D): Frozen Heart count corrected from "three implementations (Bellman, Gnark, academic reference)" to "six implementations across three proof systems (Dusk PLONK, Iden3/SnarkJS Groth16, ConsenSys/gnark PLONK, ING zkrp Bulletproofs, SECBIT ckb-zkp Groth16, Adjoint Haskell Bulletproofs)" -- aligned with ch08 canonical treatment. Applied in Layer 5 thumbnail (Trust Decomposition list) and in the "If Layer 5 fails" failure-scenario paragraph. [v2]

P1 #2 ch10-path-one-hybrid-stark-to-snark-pipeline (B): Removed the incorrect "via Herodotus" attribution (Herodotus is storage-proofs, not a wrapping provider); Stwo aggregation now attributed to SHARP only, with citation [23] to Circle STARKs paper. SP1 Hypercube cited [52]. Gas-cost dollar figures softened to order-of-magnitude claim with ethproofs.org citation [44]; removed specific $5-$25 / $0.50-$1.00 figures that could not be verified at the moment of writing. [v2]

P1 #3 ch10-path-three-post-quantum-folding (A/B): FIPS citation tightened. Module-SIS now attributed specifically to FIPS 204 (ML-DSA) [64]; FIPS 203 (ML-KEM, Module-LWE) and FIPS 205 (SLH-DSA) noted for context [25]. Claim that Module-SIS underpins folding construction preserved. [v2]

P1 #4 ch10-path-two-pure-transparent (B): EF December 2025 pivot, security targets (100-bit May 2026 / 128-bit December 2026), and energy-as-metric shift now cited [55]. SP1 Hypercube proximity-gap elimination cited [52]. Brevis Pico Prism figures reframed as "Brevis reports" with the specific 16-RTX-5090 / two-machine / 99% / ~7s numbers from the Feb 2026 Brevis blog post, per P1 allowance for reported figures. [v2]

P1 #5 ch10-causal-web-dag-not-a-stack (A): "Every layer connected to every other through at most two hops" replaced with "every layer reaches every other layer through a small number of hops," which is accurate for the edge set listed (Layer 1 to Layer 3 is three hops: 1→5→4→3). [v2]

P1 #6 ch10-trust-decomposition (A): Tornado Cash Layer 2 conflation fixed. Canonical example in the "If Layer 2 fails" paragraph is now the Zcash InternalH bug (CVE-2019-7167) -- clean Layer 2 under-constrained-circuit case. Tornado Cash retained as a secondary example using the verified pre-launch MIMC `=` vs `<==` Circom constraint bug (circomlib/Kobi Gurkan disclosure), which is a genuine Layer 2 circuit bug -- not the 2023 governance attack (Layer 7), which remains cited separately in the Layer 7 thumbnail and failure scenario. [v2]

P1 #7 ch10-trustless-vs-trust-minimized (A): Closing enumeration expanded from five to all seven layers. Added Layer 4 (arithmetization faithfully encodes computation) and Layer 5 (Fiat-Shamir soundness and correct implementation) to both the italicized rhetorical question and the follow-up (a)-(g) list, matching the section's seven-assumption framing. [v2]

P2 #1 ch10-path-one-the-hybrid-stark-to-snark-pipeline (A): Entity list in wiki section file split "Pico Prism" into [[pico]] and [[prism]] as separate entries. Fixed in ch10-path-one-the-hybrid-stark-to-snark-pipeline.md: replaced both with [[pico-prism]]. Prose text was already correct throughout. [v3]

P2 #2 ch10-path-three-post-quantum-folding (B) Greyhound citation: Added bibliography entry [65] -- Nguyen, Wilson and Gregor Seiler, "Greyhound: Fast Polynomial Commitments from Lattices," CRYPTO 2024, ePrint 2024/1293 -- to BIBLIOGRAPHY.md. Citation [65] added inline at first mention of Greyhound in the Path Three section. [v3]

P2 #3 ch10-path-three-post-quantum-folding (B) SuperNeo: Clarified in the text that SuperNeo is the non-uniform IVC extension of Neo presented in the same ePrint 2025/294 paper [21], the lattice analog of SuperNova. No separate citation needed; ref [21] now applied at the Neo/SuperNeo mention. [v3]

P2 #4 ch10-the-map-redrawn (C): DAG legend expanded to name all 8 downward/cross-cutting edges explicitly (7→6, 7→5, 4→2, 3→4, 6→7, 5→7, 2→3, 4→5) with a one-line label for each. The upward chain notation changed from "6→5→4→3→2→1" to the explicit six-edge list "6→5, 5→4, 4→3, 3→2, 2→1, 1→5" to make the count verifiable. Reading-the-diagram paragraph updated to reflect the 1→5 edge. [v3]

P2 #5 ch10-trust-decomposition-seven-weaker-assumptions (E): Added one sentence to the cascade section explaining why a Layer 6 hardness failure does not propagate to Layer 2 or Layer 4: circuits and arithmetizations are mathematical objects whose correctness is a property of their construction, independent of the hardness assumption the proof system relies on. [v3]

Length check: v2 ~4,090 words; v3 ~4,190 words; delta +2.4%, within the ±10% budget.
-->
