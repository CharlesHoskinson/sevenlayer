...
<!-- Revision ledger
v2 P1 resolutions (carried forward):
P1-1 (ch08-the-price-of-a-verdict, A): Fusaka DA capacity corrected to 4.7x (3→14 blobs). Status: resolved in v2.
P1-2 (ch08-on-chain-verification-in-2026, A): "16x" DA claim removed; consistent 4.7x figure. Status: resolved in v2.

v3 P2 resolutions:

P2-1 (ch08-the-price-of-a-verdict, A): Gas formula inconsistency. Original: "$(181 + 6L) \times 1{,}000$ gas" gives 181,000 at L=0, not ~207,700 as the table claimed. Fix: table total corrected to ~186,700 (181,000 + 4,096 + 1,600 = 186,696, rounded), formula rewritten as $181{,}000 + 7{,}160L$. Throughput sentence updated from "207,700 gas...150 to 225" to "~187,000 gas...roughly 160." Subsequent ~200,000 references updated to ~187,000 for internal consistency. Status: resolved.

P2-2 (ch08-the-price-of-a-verdict, B): DA marketplace figures unanchored. Fix: added parenthetical source notes — Celestia figures dated to "Q4 2024" with reference to Celestia cost-comparison documentation; EigenDA V2 throughput attributed to "EigenLayer documentation, V2 launch, 2024"; blob-usage stat attributed to "Dune Analytics, blob usage dashboard, June 2024." Status: resolved.

P2-3 (ch08-proof-aggregation-the-missing-layer, B): SHARP, Aligned Layer, NEBRA figures unanchored. Fix: SHARP attributed to docs.starknet.io; Aligned Layer "$11B restaked ETH" attributed to "Aligned Layer launch announcement, November 2024"; NEBRA "live August 2024" attributed to "NEBRA mainnet launch post, August 2024." Status: resolved.

P2-4 (ch08-proof-aggregation-the-missing-layer, E): Aggregation trust analysis thin; no explanation of where aggregation fits seven-layer model. Fix: opening paragraph of section rewritten to place aggregation explicitly as a Layer 7 sub-layer sitting between Layers 5-6 and the on-chain verifier; trust-analysis paragraph expanded to explain what commitments the aggregator makes that are not inherited from the underlying proof system's soundness (proof inclusion, faithful representation), and why centralization creates a governance risk requiring the same scrutiny as the verifier contract itself. Status: resolved.

P2-5 (ch08-the-deepest-symmetry, A): STARK/transparent-setup qualification missing. Fix: opening paragraph qualified — "This social requirement is specific to proof systems with a trusted setup -- Groth16, PLONK, and their cousins. STARK-based systems use transparent setups derived from public randomness: no ceremony, no toxic waste, no social trust at Layer 1." Second paragraph clarified that Layer 7 governance risk applies to all proof systems. Third paragraph (the "sandwich" passage) rewritten to distinguish: for SNARKs, two social-trust layers bookend the math; for STARKs, only the top (governance) layer remains social. Subsequent Feynman passage left untouched. Status: resolved.

P2-6 (ch08-case-study-midnight-and-the-three-token-architecture, B): No sources cited for Midnight architecture. Fix: added "The architecture described here is drawn from the Midnight developer documentation and the Compact language specification (docs.midnight.network, accessed early 2026)" to the section introduction. Status: resolved.

P2-7 (ch08-who-verifies-the-verifier, B): FOOM Club exploit uncited. Fix: added "documented in the FOOM Club post-mortem and independently confirmed in the snarkjs GitHub issue tracker, 2022" after the technical claim about delta_2 == gamma_2. Status: resolved.

Length: ~7,050 words (v2 ~6,840 words). +3%, within ±10%.
Preserved: Beanstalk $182M flash-loan narrative, Tornado Cash CREATE2 governance narrative, price-of-a-verdict gas analysis, Fiat-Shamir triple (Frozen Heart / Last Challenge Attack / Solana ZK ElGamal), L2Beat Stages, proof aggregation section, Midnight three-token / verifier-key / private governance architecture, pricing attacks (DA-saturation + prover-killer), deepest symmetry argument. Section order unchanged.
-->
