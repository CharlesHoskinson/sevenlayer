...
## v2 revisions (P1 findings, 9 items)

- P1 #1 (Sudoku arithmetic). 4 rows + 4 columns + 4 (2x2 boxes) = 12 groups, each with C(4,2) = 6 pairs, giving 12 x 6 = 72 uniqueness pair checks. Total constraints now stated as 16 range + 8 given-cell + 72 uniqueness = 96, explicitly walked through for the reader. The R1CS expansion estimate was revised proportionally: 16 degree-4 range checks decompose into ~3 bilinear rows each (~48), plus 72 uniqueness pair-difference rows, plus 8 equality rows, landing at roughly 130 R1CS constraints. The "~120" phrase is replaced with "roughly 130".
- P1 #2 (Groth16 proof size). Both the body and the summary now state 192 bytes on BLS12-381. Source added to the Sources list.
- P1 #3 (AIR table misleading). "Same Computation, Three Encodings" table restructured: description size vs. prover trace work split into separate columns.
- P1 #4 (Jolt 64-bit canonical). 64-bit RISC-V case fixed as canonical (18 field elements). 32-bit case mentioned once for contrast.
- P1 #5 (LogUp-GKR attribution). Full author names added for Plookup and LogUp-GKR citations.
- P1 #6 (impact opcode). All four I/O opcodes defined in one place.
- P1 #7 (BLS12-381 field width). "255-bit prime with order approximately $2^{255}$" used consistently.
- P1 #8 (Airbender row). Table header now states "vs ~100 ms block" baseline; Airbender row corrected to ~350x.
- P1 #9 (Midnight shielded transfer). 20 s / 20 us = 1,000,000x figure and baseline stated explicitly.

## v3 revisions (P2 findings, 23 items)

- P2 #1 (Neo/LatticeFold). Neo bullet revised to "a lattice-based folding scheme targeting CCS natively" with parenthetical clarifying LatticeFold as predecessor; "first lattice-based folding scheme for CCS" claim removed. RESOLVED.
- P2 #2 (Hadamard notation). Display formula changed from $\bigcirc$ to $\circ$ (using indexed subscript form); prose definition updated to match. RESOLVED.
- P2 #3 (Penrose name-drop). Sentence beginning "There is a deeper mathematical point here, one that Penrose would appreciate" — Penrose name-drop removed; point stands without it. RESOLVED.
- P2 #4 (AI-smell "key insight"). "here is the central move that makes the entire field of zero-knowledge proofs possible" reworded as "The move that makes this verifiable:" — direct statement, no AI-smell qualifier. RESOLVED.
- P2 #5 (Schwartz-Zippel in body). Schwartz-Zippel lemma now named and stated in the core-mechanism paragraph where first invoked, not only in metadata. RESOLVED.
- P2 #6 (Blum et al. citation). Full citation added: Blum, Evans, Gemmell, Kannan, Naor, "Checking the Correctness of Memories," FOCS 1991 — both in prose and as [R-L4-11] in Sources. RESOLVED.
- P2 #7 (LogUp derivative transition). "Differentiating both sides gives" inserted between the log identity and the displayed P'/P formula; inline notation switched to formal $\prod$ and $\log$ to make derivation explicit. RESOLVED.
- P2 #8 (Midnight Halo2 citation). Inline reference now names "Compact compiler (v0.29.0)" and "Midnight ZKIR Reference v2/v3" at point of use. RESOLVED.
- P2 #9 (under-constrained duplicate). Paragraph restating the under-constrained principle replaced with cross-reference: "This is an instance of the under-constrained circuit principle examined in Chapter 3." RESOLVED.
- P2 #10 (R1CS/QAP history). Added Pinocchio (Parno et al., 2013) as the paper that standardized the R1CS matrix form; text now distinguishes QAP (GGPR 2012) from the R1CS reformulation (Pinocchio 2013). RESOLVED.
- P2 #11 (duplicate "history of arithmetization"). Opening paragraph of constraint-evolution section collapsed from two overlapping sentences into one; second "history of arithmetization is a history of expressiveness" sentence removed. RESOLVED.
- P2 #12 (Overhead Tax source dominance). Added explicit distinction: constraint expansion causes the largest multiplicative overhead relative to native execution; polynomial commitment causes the largest absolute wall-clock cost. RESOLVED.
- P2 #13 (Overhead benchmarks citations). Ethereum-block proof-time claim now cites SP1 v2 blog post, RISC Zero v1.0 benchmark, and Stwo benchmark announcement inline. RESOLVED.
- P2 #14 (interpolation/Schwartz-Zippel conflation). "Why polynomials?" paragraph rewritten to separate the two distinct facts: interpolation uniqueness (reconstruction) vs. Schwartz-Zippel (probabilistic detection). RESOLVED.
- P2 #15 (core idea twice). Already resolved in v2; only one occurrence remains. SKIPPED.
- P2 #16 (SP1 Hypercube citation). Already resolved in v2 — "(Succinct, 2025)" present in bullet. SKIPPED.
- P2 #17 (sumcheck NTT qualification). "avoids NTTs entirely" qualified: sumcheck itself is NTT-free; the surrounding commitment scheme may vary. Hyrax/Brakedown cited as NTT-free multilinear schemes. RESOLVED.
- P2 #18 (internal combustion analogy). Analogy sentence removed; point restated plainly. RESOLVED.
- P2 #19 (Jolt 6x citation). Reference Data entry updated: "5x-10x" range from the Jolt paper (ePrint 2023/1217) cited; "~6x" identified as the paper's own representative figure. RESOLVED.
- P2 #20 (lower bound misleading). Paragraph rewritten to clarify the bound applies to communication, not proof size; SNARKs routinely produce proofs far smaller than the output. RESOLVED.
- P2 #21 (Sudoku uniqueness in closing). "misleads about uniqueness" note in closing section now carries "(as noted at the opening of this chapter)" to flag it as a recap, not a new assertion. RESOLVED.
- P2 #22 (RISC Zero R1CS claim). Corrected: RISC Zero v0.x used a 254-bit field STARK with AIR, not R1CS/Groth16. Migration was from one STARK configuration to another. RESOLVED.
- P2 #23 (Goldilocks/lattice). Proof-core triad bullet corrected: LatticeFold targets general fields via NTRU lattices, not Goldilocks specifically. Neo separately noted as Goldilocks-targeted but sumcheck-based. RESOLVED.
