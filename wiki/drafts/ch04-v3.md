...
P2 findings resolved in this draft (v3), against ch04.md brief:

1. **[ch04-execution-traces](B)** SP1/RISC Zero billions-of-elements claim: added "SP1's benchmark suite and RISC Zero's zkVM documentation both describe multi-billion-element traces for large EVM workloads" as the anchor.

2. **[ch04-execution-traces](A)** ~80 field elements derivation: expanded the Sudoku box to show the accounting explicitly -- 16 cell values + 12 groups × 6 pairwise comparisons = 88, approximated as ~80.

3. **[ch04-execution-traces](C)** "Running Example:" textbook label: changed callout title from "The Running Example: The Sudoku Witness" to "The Sudoku Witness".

4. **[ch04-memory-the-binding-constraint](A)** 5.5B smartphone figure: replaced with GSMA Intelligence (2025) figure of ~4.6 billion unique smartphone subscribers; changed "96%" to "well over 90%".

5. **[ch04-memory-the-binding-constraint](B)** Crypto Wars 40-bit citation: added "under the Export Administration Regulations" and cited "the EFF's *Cracking DES*, 1998" and RFC 6176 as anchors for the 40-bit cap claim.

6. **[ch04-memory-the-binding-constraint](C)** "Read that again." imperative: deleted; argument stands in the following sentence.

7. **[ch04-memory-the-binding-constraint](C)** Doubled "uncomfortable" framing: changed "uncomfortable conclusion" to "stark conclusion"; "uncomfortable math:" paragraph opens with "The numbers are sobering." instead.

8. **[ch04-memory-the-binding-constraint](E)** TEE vulnerability names without context: added brief descriptions and years -- Foreshadow (L1TF, 2018), AEPIC Leak (2022, architectural APIC leak), Downfall (GDS, 2023, AVX gather instructions).

9. **[ch04-side-channel-attacks](A)** EM ZK MSM extrapolated not demonstrated: revised to "can, in principle, reconstruct" and added "No published paper has demonstrated this attack end-to-end against a ZK MSM implementation specifically; the extrapolation from established EM attacks on AES and ECDSA is technically grounded but not yet empirically closed for ZK provers."

10. **[ch04-side-channel-attacks](C)** "detective story" genre label: removed the sentence "The Zcash attack deserves to be told as the detective story it was, because the investigative method reveals..." Subsection retitled from "The Zcash Timing Attack: A Detective Story" to "The Zcash Timing Attack".

11. **[ch04-the-disclose-boundary](B)** Midnight developer guide no date: added "(Midnight developer forum, 2025)" attribution to the timing figures; the quote "Witnesses stay local. Never sent to chain." retains its attribution note.

12. **[ch04-the-disclose-boundary](A)** BIP-340 claim: replaced "BIP-340 Schnorr signatures" with "Schnorr signatures (Midnight uses a Cardano-native Schnorr scheme over the Ed25519 curve, distinct from Bitcoin's BIP-340)".

13. **[ch04-the-disclose-boundary](C)** "Privacy on Midnight is genuine... unexamined" without preceding argument: added a cross-reference sentence: "As established in the Side-Channel Attacks section above, these implementation-level channels exist independently of the cryptographic proof's zero-knowledge property." Paragraph then draws the evaluative conclusion.

14. **[ch04-the-hidden-bottleneck](A)** "commonly claimed" vague attribution: revised to "Not 10-25% -- a figure that was roughly correct in 2023, when the proving step was slow enough to dwarf everything else." This pins the period explicitly. The Witness Generation Costs section opens with the same framing for consistency.

15. **[ch04-the-hidden-bottleneck](C)** "Now the magician goes backstage" theatrical repetition from ch03: changed to "Costs begin." -- one sentence, drops the ch03 frame restart.

16. **[ch04-the-witness-as-a-multi-dimensional-problem](A)** "~96%" repeated verbatim: changed to reference the GSMA-grounded figure established earlier ("The GSMA-grounded figure above -- well over 90%...") rather than restating the number.

17. **[ch04-the-witness-as-a-multi-dimensional-problem](C)** "The mathematics is beautiful. The engineering is brutal." stock aphorism: deleted; the section ends with "Layer 3 is where the practical reality of zero-knowledge systems diverges most sharply from the elegant theory." which carries the same weight without the AI-smell contrast.

18. **[ch04-the-witness-as-a-multi-dimensional-problem](D)** Equity gap as "fifth dimension" inconsistency: reframed as "an equity consequence of the Memory Gap" rather than a peer fifth dimension; the numbering inconsistency is resolved by making it explicitly downstream of the Memory Gap.

19. **[ch04-witness-constraint-divergence](A)** ZKAP F1 score task unspecified: clarified to "F1 score 0.82 on under-constrained circuit detection" in both the Closing the Correctness Gap subsection and the synthesis.

20. **[ch04-witness-constraint-divergence](B)** NAVe and Picus no citations: added "(Ozdemir, Brown, and Bonneau, 2023; targeted at Noir)" for NAVe and "(Wen, Cheng, and Wang, CCS 2023; targeted at Circom)" for Picus.

21. **[ch04-witness-constraint-divergence](C)** "lines of code not written" imprecision: replaced with "The zkSync bug illustrates the typical failure mode: the method enforcing the constraint existed but was never called. Missing constraints are not always missing code; they are sometimes code present but never invoked."

22. **[ch04-witness-generation-costs](A)** RAM estimate missing field size: added "in 64-bit Goldilocks field elements (8 bytes each)... systems using 254-bit BN254 elements would require roughly four times as much."

23. **[ch04-witness-generation-costs](B)** Source table missing publication years: added years to all five entries -- ZKPoG (2025), ZKProphet (2025), BatchZK (2025), Nair/Thaler/Zhu (ePrint 2025/611), Ozdemir/Laufer/Boneh (IEEE S&P 2025, ePrint 2024/979).

24. **[ch04-witness-generation-costs](C)** "Welcome to the Witness Gap" exclamatory: changed to "The Witness Gap is growing, not shrinking." -- declarative, no em-dash opener.

Already-resolved items carried forward from v2 (P0 and P1 items): Poseidon S-box accuracy, memory figures split, hardware price dating, structural echo fix, Tramer-Boneh-Paterson title, Midnight timing rewrite, 18-second benchmark replaced, ZKPoG first-mention citation, CVE-2025-52484 instruction class, zkFuzz citation, 3,040x consistency, streaming bound reconciliation.
