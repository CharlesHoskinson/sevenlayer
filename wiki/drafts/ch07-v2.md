...
**Resolved findings (8/8):**

1. **P0 ch07-small-fields** -- BLS12-381 scalar field bitlength. Replaced "254 and 253 bits respectively" / "254-bit multiplication" language with the correct "255-bit scalar field $r \approx 2^{254}$ (base field $p$ is 381 bits)." Consistent with ch02 and ch05.

2. **P0 ch07-case-study-midnight** -- BLS12-381 scalar field bitlength. Replaced "~253-bit prime modulus $r$" with "255-bit prime scalar field $r \approx 2^{254}$" in the Midnight stack description. Comparison table updated to "BLS12-381, 255-bit scalar."

3. **P0 ch07-the-one-way-door** -- BLS12-381 bitlength in the architect's decision tree and rubric table. Replaced "254-bit pairing-friendly curve" / "a single 254-bit multiplication" with "255-bit scalar field, 381-bit base field, pairing-friendly" / "a single multiplication in the 255-bit scalar field." Rubric table entry now reads "255-bit scalar / 381-bit base."

4. **P1 ch07-algebraic-vs-traditional-hash-functions** -- Rewrote the side-channel paragraph. Now states that Poseidon uses a power-map S-box ($x \mapsto x^\alpha$) as straight-line field arithmetic, with no secret-indexed memory access. Attributes the Mukherjee cache-timing risk to table-based designs (specifically Reinforced Concrete's Bars decomposition), not to power-map hashes. Notes that engineers substituting tables for direct exponentiation in a Poseidon implementation can reintroduce the risk. Consistent with ch04 correction.

5. **P1 ch07-lattice-based-proving** -- LatticeFold date disambiguated. Stage-2 heading now reads "LatticeFold (Boneh-Chen; ePrint 2024/257, published at ASIACRYPT 2025)." Same form used in Maturity and Readiness section. Consistent with ch06 revision and bibliography entry.

6. **P1 ch07-the-structural-advantage-of-lattices** -- Zexe replaced with Halo as the canonical pre-quantum recursive example, with Halo2/Zcash and Pickles/Mina named as production descendants. Pasta pair (Pallas/Vesta) cited as the specific curve cycle. Third bullet rewritten to acknowledge Halo's transparent setup while noting parameter complexity from maintaining two curves.

7. **P1 ch07-the-trilemma-and-its-dissolution** -- Opening line now reads: "I call this shape of tradeoffs the *cryptographic primitives trilemma* -- my own framing, not a result from the literature." Original authorial framing made explicit.

8. **P1 ch07-three-hardness-assumptions-three-worlds** -- BHT citation added inline: "[Brassard, Høyer, Tapp, 'Quantum cryptanalysis of hash and claw-free functions,' LATIN 1998, LNCS 1380, pp. 163--169; arxiv quant-ph/9705002]."

**Sources verified/added:**
- Brassard, Høyer, Tapp, "Quantum cryptanalysis of hash and claw-free functions," LATIN 1998, LNCS 1380, pp. 163--169. DOI 10.1007/BFb0054319. arXiv: quant-ph/9705002 (1997 preprint, ACM SIGACT News v28n2).
- Bowe, Grigg, Hopwood, "Halo: Recursive Proof Composition without a Trusted Setup," Cryptology ePrint Archive 2019/1021 (2019).

**Unresolved:** none. All 3 P0 + 5 P1 findings addressed.
