...
**P2 items resolved (v3):**

1. [ch02-bn254-s-eroding-security-margin B] Added full titles and venues: Kim & Barbulescu → "Extended Tower Number Field Sieve: A New Complexity for the Medium Prime Case," *CRYPTO 2016*; Menezes, Sarkar, Singh → "Challenges with Assessing the Impact of NFS Advances on the Security of Pairing-Based Cryptography," *MathCrypt 2016*.

2. [ch02-midnight-s-bls12-381-stage B] Added source note "[Midnight Network devnet documentation, docs.midnight.network, as of Q1 2025]" for the 13.7 KB / 1.3 KB key sizes and the localhost:6300 proof-server URL.

3. [ch02-option-value-analysis B] Added specific Snowden document reference: Barton Gellman and Laura Poitras, *Washington Post*, June 2013, for the NSA upstream collection program disclosure.

4. [ch02-option-value-analysis C] Reframed HNDL paragraph: removed "it may be the most urgent variant" and replaced with a qualified statement ("For systems with 10+ year lifespans, it may be the most pressing near-term exposure") that specifies the relevant time horizon.

5. [ch02-the-141-416-person-question B] Added Luhmann citation: *Risk: A Sociological Theory*, 1993 — the work most directly relevant to confidence in systems rather than interpersonal trust.

6. [ch02-the-141-416-person-question C] Removed "it is worth noting" from the game-theoretic analysis paragraph (phrase was not in v2; the surviving instance was the subsection heading). Renamed subsection "The Sociology of Trust at Scale" → "The Structure of Ceremony Trust" for specificity.

7. [ch02-the-adopt-framework B] Already resolved in v2 — Wang, Cohney, Bonneau citation carries "Financial Cryptography 2025 / IACR ePrint 2025/064." No change needed.

8. [ch02-the-adopt-framework C] Removed the leading-italics "Their finding: *no existing ceremony...*" construction; replaced with plain declarative: "No existing ceremony satisfies all five properties."

9. [ch02-the-bug-that-was-not-a-ceremony-failure B] Added BCTV14 original paper citation: Bitansky, Chiesa, Tromer, Virza, "Succinct Non-Interactive Arguments via Linear Interactive Proofs," *TCC 2013*, with a note that the 2014 extension is the Zcash Sprout deployment.

10. [ch02-the-capex-opex-framework A] Added a sentence anchoring the gas estimates: "The figures below are Ethereum mainnet estimates as of early 2026; gas prices fluctuate, so these should be read as order-of-magnitude anchors rather than billing rates."

11. [ch02-the-capex-opex-framework C] Replaced "Smaller than a tweet" with "compact enough to fit in a short text message" to vary the phrase's repetition across the wiki.

12. [ch02-the-fair-shuffle-problem A] Fixed the "add a bit of randomness" framing to "contribute a secret factor" with explicit explanation that contributions multiply and any single honest factor makes the product unguessable, correcting the additive misconception before the algebra section.

13. [ch02-the-fair-shuffle-problem B] Added footnote-style citation for the 141,416 participant count: Ethereum Foundation, "KZG Summoning Ceremony," ceremony.ethereum.org, 2023, with reference to the publicly verifiable transcript.

14. [ch02-the-fair-shuffle-problem C] Replaced "glass on the inside, paper on the outside" with "transparent on the inside, compact on the outside," removing the fragile-glass connotation while preserving the contrast.

15. [ch02-the-quantum-shelf-life B] Already resolved in v2 — Neo citation carries "IACR ePrint 2025/294." No change needed.

16. [ch02-the-quantum-shelf-life C] Replaced "The honest assessment:" with "In sum:" — removes the AI-smell framing.

17. [ch02-the-setup-tradeoff A] Softened "every major production system has chosen" to "That architecture describes the dominant production pattern in 2026," and added a parenthetical in the Capex/Opex section explicitly noting that Polygon's Plonky2/Plonky3 wraps to PLONK rather than Groth16.

18. [ch02-the-setup-tradeoff C] Already resolved in v2 — Part II header divider does not appear in the section file. No change needed.

19. [ch02-the-structured-reference-string C] Trimmed the finite field aside by one sentence (removed "In practice, the 'clock' has billions or trillions of hours -- large enough that the wrapping-around property provides the randomness the proof system needs to catch cheaters"), keeping the core clock-arithmetic definition without the extended elaboration.

20. [ch02-two-ways-to-build-a-stage C] (a) Replaced "the property to hold onto is that pairings let the verifier..." with "for now: pairings let the verifier..." removing the "key property" pattern. (b) Removed the duplicate opening of the second "ceremony did not need" paragraph; the first paragraph now stands alone, and the second paragraph begins with "A ritual's efficacy..." without restating the same claim.

21. [ch02-universal-versus-circuit-specific-setups A] Replaced "verified with three pairing operations" with precise language: "Verification requires two pairing checks, each involving two pairing evaluations -- three distinct pairing computations in all, with one argument shared across checks."

22. [ch02-universal-versus-circuit-specific-setups B] Already resolved in v2 — Marlin citation carries "EUROCRYPT 2020 / IACR ePrint 2019/1047." No change needed.

23. [ch02-universal-versus-circuit-specific-setups C] Rewrote the Midnight transition paragraph to cut the stalling bridge ("The Sudoku puzzle from Chapter 1 will carry us through..."). Replaced with a direct sentence: "Midnight is the production system this chapter uses as its running case study: it adopted the universal model with a PLONK-family proof system (Halo 2 / UltraPlonk) on BLS12-381."
